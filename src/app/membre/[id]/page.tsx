...
function Sparkline({ notes }: { notes: number[] }) {
  ...
  const points = notes.map((n, i) => {
    const x = pad + (i / (notes.length - 1)) * (W - 2 * pad);
    const y = H - pad - ((n - 1) / 4) * (H - 2 * pad);
...
export default async function FicheMembre({
  ...
  const [
    ...
    supabase
      .from("visites")
      .select("id, date_realisee, note_confiance, note_business, objectif, points_cles")
      .eq("magasin_id", id)
      .eq("statut", "realisee")
    ...
    supabase
      ...
      .select("q6_satisfaction_globale, created_at")
      .eq("magasin_id", id)
      .order("created_at", { ascending: false })
      .limit(5),
    supabase
      .from("remontees")
      .select("id, titre, gravite, statut, created_at")
      .eq("magasin_id", id)
  ...
  const notesCfn = trois.map((v) => v.note_confiance).filter(Boolean) as number[];
  const notesBiz = trois.map((v) => v.note_business).filter(Boolean) as number[];
  const notesEval = (evaluations ?? []).map((e) => e.q6_satisfaction_globale).filter(Boolean) as number[];

  const moy = (arr: number[]) =>
    arr.length > 0 ? (arr.reduce((a, b) => a + b, 0) / arr.length).toFixed(1) : null;

  const moyCfn = moy(notesCfn);
  const moyBiz = moy(notesBiz);
  const moySat = moy(notesEval);

  // Sparkline : notes confiance dans l'ordre chronologique
  const sparkNotes = [...dernieresVisites]
    .reverse()
    .map((v) => v.note_confiance)
    .filter(Boolean) as number[];

  const nomAffiche = magasin.enseigne ?? magasin.nom;
  const urgenceLabel: Record<number, string> = { 1: "Info", 2: "Important", 3: "Urgent" };
  const urgenceColor: Record<number, string> = {
    1: "bg-slate-100 text-slate-600",
    2: "bg-amber-100 text-amber-700",
    3: "bg-red-100 text-red-700",
  };
  const graviteBadge: Record<string, string> = {
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="text-slate-500 mt-1 text-sm">
                ...
                {magasin.nom !== nomAffiche ? " · " : ""}
                {magasin.ville}
                {magasin.region ? ` · ${magasin.region}` : ""}
              </p>
              {meteo && (
                <p className="mt-2 text-slate-700 text-sm font-medium">
                  {meteo.temp}°C — {meteo.libelle}
                </p>
              )}
            </div>
            <div className="flex flex-col items-end gap-2 shrink-0 text-right">
              ...
              <ChangerRole className="text-sm text-slate-400 hover:text-slate-700 transition-colors" />
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 text-center">
              ...
              <div className="text-xs text-slate-500 mt-1">Confiance moy.</div>
              <div className="text-xs text-slate-400">3 dernières visites</div>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 text-center">
              <div className="text-2xl font-bold text-slate-900">
                {moyBiz ? `${moyBiz}/5` : "—"}
              </div>
              <div className="text-xs text-slate-500 mt-1">Business moy.</div>
              <div className="text-xs text-slate-400">3 dernières visites</div>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 text-center">
              <div className="text-2xl font-bold text-slate-900">
              ...
              <div className="text-xs text-slate-400">évaluations reçues</div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          {trois.length === 0 ? (
            <div className="bg-white rounded-xl border border-slate-200 p-8 text-center text-slate-400 text-sm shadow-sm">
              Aucune visite enregistrée pour ce magasin.
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-slate-700 truncate">
                      {v.objectif ?? "Pas d'objectif renseigné"}
                    </p>
                    {v.points_cles && (
                      <p className="text-xs text-slate-400 mt-0.5 truncate">{v.points_cles}</p>
                    )}
                  </div>
                  <div className="shrink-0 flex gap-3 text-xs text-slate-500">
                    {v.note_confiance != null && (
                      <span>Conf. <strong className="text-slate-700">{v.note_confiance}/5</strong></span>
                    )}
                    {v.note_business != null && (
                      <span>Biz. <strong className="text-slate-700">{v.note_business}/5</strong></span>
                    )}
                  </div>
                </div>
              ))}
          ...
          {(actions ?? []).length === 0 ? (
            ...
            <div className="space-y-2">
              {(actions ?? []).map((a) => (
                ...
                >
                  <span
                    className={`shrink-0 text-xs font-semibold px-2 py-0.5 rounded-full ${urgenceColor[a.niveau_urgence] ?? "bg-slate-100 text-slate-600"}`}
                  >
                    {urgenceLabel[a.niveau_urgence] ?? "—"}
                  </span>
          ...
          <div>
            ...
            <div className="space-y-2">
              {(remontees ?? []).map((r) => (
                ...
                >
                  <span
                    className={`shrink-0 text-xs font-semibold px-2 py-0.5 rounded-full ${graviteBadge[r.gravite] ?? "bg-slate-100 text-slate-600"}`}
                  >
